...
class AlbersUsaProjection:
    ...
    def __init__(self):
        ...
        self._alaska = geo_conic_equal_area().rotate([154, 0]).set_center([-2, 58.5]).parallels([55, 65])
        self._alaska_point = None
        self._hawaii = geo_conic_equal_area().rotate([157, 0]).set_center([-3, 19.9]).parallels([8, 18])
    ...
    def stream(self, stream: PointStream):
        if self._cache is not None and self._cache_stream == stream:
            ...
            self._cache = Multiplex([self._lower48.stream(stream), self._alaska.stream(stream), self._hawaii.stream(stream)])
            return self._cache
    ...
    def translate(self, translation: Vec2D) -> TAlbersUsaProjection:
        ...
        self._lower48_point = self._lower48.translate(translation).set_clip_extent([[x - 0.455 * k, y - 0.238 * k], [x + 0.455 * k, y + 0.238 * k]]).stream(self._point_stream)

        self._alaska_point = self._alaska.translate([x - 0.307 * k, y + 0.201 * k]).set_clip_extent([[x - 0.425 * k + EPSILON, y + 0.120 * k + EPSILON], [x - 0.214 * k - EPSILON, y + 0.234 * k - EPSILON]]).stream(self._point_stream)

        self._hawaii_point = self._hawaii.translate([x - 0.205 * k, y + 0.212 * k]).set_clip_extent([[x - 0.214 * k + EPSILON, y + 0.166 * k + EPSILON], [x - 0.115 * k - EPSILON, y + 0.234 * k - EPSILON]]).stream(self._point_stream)

        return self.reset()

    def fit_extent(self, extent: tuple[Point2D, Point2D], obj: GeoJSON) -> TAlbersUsaProjection:
