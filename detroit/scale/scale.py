...
def normalize(a, b):
    return (b := (b - (a := float(a)))) and (lambda x: (x - a) / b) or constant(float('nan') if math.isnan(b) else 0.5)
...
def transformer():
    ...
    def scale(x):
        return unknown if x is None or (isinstance(x, float) and math.isnan(x)) else (output or (output := piecewise(list(map(transform, domain)), range_vals, interpolate)))(transform(clamp(x)))

    def invert(y):
        return clamp(untransform((input or (input := piecewise(range_vals, list(map(transform, domain)), interpolate_number)))(y)))
...
# src/utcTime.py
from d3_time import utcYear, utcMonth, utcWeek, utcDay, utcHour, utcMinute, utcSecond, utcTicks, utcTickInterval
from d3_time_format import utcFormat
from time import calendar
from init import init_range

def utc_time():
    return init_range(calendar(utcTicks, utcTickInterval, utcYear, utcMonth, utcWeek, utcDay, utcHour, utcMinute, utcSecond, utcFormat).domain([datetime.datetime(2000, 1, 1), datetime.datetime(2000, 1, 2)]))
...
# src/time.py
from d3_time import timeYear, timeMonth, timeWeek, timeDay, timeHour, timeMinute, timeSecond, timeTicks, timeTickInterval
...
def number(t):
    return t if isinstance(t, datetime.datetime) else datetime.datetime.fromtimestamp(t).timestamp()

def calendar(ticks, tick_interval, year, month, week, day, hour, minute, second, format):
    scale = continuous()
    invert = scale.invert
    domain = scale.domain
    ...
    def tick_format(date):
        return (format_millisecond if second(date) < date else
                format_second if minute(date) < date else
                format_minute if hour(date) < date else
                format_hour if day(date) < date else
                format_week if month(date) < date else
                format_month if year(date) < date else
                format_year)(date)
    ...
    def domain_func(_=None):
        return domain(list(map(number, _))) if _ is not None else list(map(date, domain()))
    ...
    def nice_func(interval=None):
        ...
        if not interval or not hasattr(interval, 'range'):
            interval = tick_interval(d[0], d[-1], interval if interval is not None else 10)
        return interval and domain(nice(d, interval)) or scale

    def copy_func():
        return copy(scale, calendar(ticks, tick_interval, year, month, week, day, hour, minute, second, format))
...
def time():
    return init_range(calendar(timeTicks, timeTickInterval, timeYear, timeMonth, timeWeek, timeDay, timeHour, timeMinute, timeSecond, timeFormat).domain([datetime.datetime(2000, 1, 1), datetime.datetime(2000, 1, 2)]))
...
from d3_format import format, format_prefix, format_specifier, precision_fixed, precision_prefix, precision_round
