...
from detroit import Data, Script, Theme, d3, function, js, render, save, svg
...
def make_script(script):
    ...
    ymax = max(
        data.allvalues[len(data.allzeta.data) - 1].values.data, key=lambda d: d["y"]
    )["y"]
...
scripts = {
    f"Title {i}": make_script(script) for i, script in enumerate(Script.multiple(4))
}
