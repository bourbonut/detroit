...
def test_pow_3():
    assert d3.scale_pow().set_domain([-10, 0]).set_range(["red", "white", "green"]).set_clamp(True)(-5) == "rgb(255, 128, 128)"
    assert d3.scale_pow().set_domain([-10, 0]).set_range(["red", "white", "green"]).set_clamp(True)(50) == "rgb(255, 255, 255)"

def test_pow_4():
    assert d3.scale_pow().set_domain([-10, 0, 100]).set_range(["red", "white"]).set_clamp(True)(-5) == "rgb(255, 128, 128)"
    assert d3.scale_pow().set_domain([-10, 0, 100]).set_range(["red", "white"]).set_clamp(True)(50) == "rgb(255, 255, 255)"
...
def test_pow_10():
    ...
    assert d3.scale_pow().set_range([datetime(1990, 1, 1), datetime(1991, 1, 1)]).invert(datetime(1990, 7, 2, 13)), 0.5
...
def test_pow_15():
    assert d3.scale_pow().set_domain([datetime(1990, 1, 1), datetime(1991, 1, 1)]).domain == [datetime(1990, 1, 1), datetime(1991, 1, 1)]
...
def test_pow_19():
    ...
    assert d3.scale_pow().set_range(["rgb(255,0,0)", "hsl(240,100%,50%)"])(0.5) == "rgb(128, 0, 128)"
    assert d3.scale_pow().set_range(["rgb(100%,0%,0%)", "hsl(240,100%,50%)"])(0.5) == "rgb(128, 0, 128)"
    assert d3.scale_pow().set_range(["hsl(0,100%,50%)", "hsl(240,100%,50%)"])(0.5) == "rgb(128, 0, 128)"

def test_pow_20():
    assert d3.scale_pow().set_range([{"color": "red"}, {"color": "blue"}])(0.5) == {"color": "rgb(128, 0, 128)"}
    assert d3.scale_pow().set_range([["red"], ["blue"]])(0.5) == ["rgb(128, 0, 128)"]
...
def test_pow_28():
    def interpolate(a, b):
        def f(t):
            return [a, b, t]
        return f
    s = d3.scale_pow().set_domain([10, 20]).set_range(["a", "b"]).set_interpolate(interpolate)
...
def test_pow_32():
    assert d3.scale_pow().set_domain([1.1, 1, 2, 3, 10.9]).nice(10).domain == [1, 1, 2, 3, 11]
    assert d3.scale_pow().set_domain([123.1, 1, 2, 3, -0.9]).nice(10).domain == [130, 1, 2, 3, -10]
...
def test_pow_41():
    ...
    assert d3.scale_pow().set_domain([0.19, 0.21]).tick_format(10, "+%")(0.2) == "+20.0%"
