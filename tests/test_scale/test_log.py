...
def test_log_6():
    ...
    assert d3.scale_log().set_domain([-0.15, -0.68]).ticks() == [
        -0.2,
        -0.3,
        -0.4,
        -0.5,
    ...
    assert d3.scale_log().set_domain([-0.68, -0.15]).ticks() == [
        -0.6,
        -0.5,
        -0.4,
        -0.3,
...
def test_log_17():
    ...
    assert list(map(x.tick_format(), x.set_base(2).ticks())) == [
        "1",
        "2",
        "4",
        "8",
    ...
    assert list(map(x.tick_format(), x.set_base(math.e).ticks())) == [
        "1",
        "2.71828182846",
        "7.38905609893",
        "20.0855369232",
...
def test_log_26():
    s = d3.scale_log()
    assert list(map(partial(round, ndigits=1), s.set_domain([1e-1, 1e1]).ticks())) == [
        0.1,
        0.2,
        0.3,
        0.4,
    ...
    assert list(map(partial(round, ndigits=1), s.set_domain([1e-1, 1e0]).ticks())) == [
        0.1,
        0.2,
        0.3,
        0.4,
    ...
    assert list(
        map(partial(round, ndigits=1), s.set_domain([-1e0, -1e-1]).ticks())
...
def test_log_27():
    s = d3.scale_log()
    assert (
        list(map(partial(round, ndigits=1), s.set_domain([-1e-1, -1e1]).ticks()))
        == [
            -10,
            -9,
            -8,
    ...
    assert (
        list(map(partial(round, ndigits=1), s.set_domain([-1e-1, -1e0]).ticks()))
        == [-1, -0.9, -0.8, -0.7, -0.6, -0.5, -0.4, -0.3, -0.2, -0.1][::-1]
    )
    assert (
        list(map(partial(round, ndigits=1), s.set_domain([1e0, 1e-1]).ticks()))
...
def test_log_28():
    ...
    assert s.set_domain([286.9252014, 329.4978332]).ticks() == [
        290,
        295,
        300,
        305,
...
def test_log_29():
    s = d3.scale_log()
    assert s.set_domain([41, 42]).ticks() == [
        41,
        41.1,
        41.2,
        41.3,
    ...
    assert s.set_domain([42, 41]).ticks() == [
        42,
        41.9,
        41.8,
        41.7,
    ...
    assert s.set_domain([1600, 1400]).ticks() == [
        1600,
        1580,
        1560,
        1540,
...
def test_log_30():
    ...
    assert list(map(partial(round, ndigits=12), s.set_domain([0.1, 100]).ticks())) == [
        0.135335283237,
        0.367879441171,
        1,
        2.718281828459,
...
def test_log_31():
    s = d3.scale_log()
    assert list(map(s.tick_format(), s.set_domain([1e-1, 1e1]).ticks())) == [
        "100m",
        "200m",
        "300m",
        "400m",
...
def test_log_32():
    ...
    assert list(map(s.tick_format(10), t)) == [
        "100m",
        "200m",
        "300m",
        "400m",
    ...
    assert list(map(s.tick_format(5), t)) == [
        "100m",
        "200m",
        "",
        "",
    ...
    assert list(map(s.tick_format(1), t)) == [
        "100m",
        "",
        "",
        "",
    ...
    assert list(map(s.tick_format(0), t)) == [
        "100m",
        "",
        "",
        "",
...
def test_log_38():
    ...
    assert list(map(x.tick_format(), x.ticks())) == [
        "10G",
        "1G",
        "100M",
        "10M",
