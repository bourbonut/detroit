...
def test_linear_5():
    assert (
        d3.scale_linear().set_domain([-10, 0]).set_range([0, 1, 2]).set_clamp(True)(-5)
        == 0.5
    )
    assert (
        d3.scale_linear().set_domain([-10, 0]).set_range([0, 1, 2]).set_clamp(True)(50)
        == 1
    )
...
def test_linear_13():
    assert d3.scale_linear().set_domain([]).get_domain() == []
    assert d3.scale_linear().set_domain([1, 0]).get_domain() == [1, 0]
    assert d3.scale_linear().set_domain([1, 2, 3]).get_domain() == [1, 2, 3]


def test_linear_14():
    assert d3.scale_linear().set_domain(
        [datetime(1990, 1, 1), datetime(1991, 1, 1)]
    ).get_domain() == [datetime(1990, 1, 1), datetime(1991, 1, 1)]
    assert d3.scale_linear().set_domain(["0.0", "1.0"]).get_domain() == [0, 1]
    assert d3.scale_linear().set_domain([0, 1]).get_domain() == [0, 1]


def test_linear_15():
    assert d3.scale_linear().set_domain({1, 2}).get_domain() == [1, 2]
...
def test_linear_19():
    assert d3.scale_linear().set_range({1, 2}).get_range() == [1, 2]


def test_linear_20():
    assert d3.scale_linear().set_range(["red", "blue"])(0.5) == "rgb(128, 0, 128)"
    assert (
        d3.scale_linear().set_range(["#ff0000", "#0000ff"])(0.5) == "rgb(128, 0, 128)"
    )
    assert d3.scale_linear().set_range(["#f00", "#00f"])(0.5) == "rgb(128, 0, 128)"
    assert (
        d3.scale_linear().set_range(["rgb(255,0,0)", "hsl(240,100%,50%)"])(0.5)
        == "rgb(128, 0, 128)"
    )
    assert (
        d3.scale_linear().set_range(["rgb(100%,0%,0%)", "hsl(240,100%,50%)"])(0.5)
        == "rgb(128, 0, 128)"
    )
    assert (
        d3.scale_linear().set_range(["hsl(0,100%,50%)", "hsl(240,100%,50%)"])(0.5)
        == "rgb(128, 0, 128)"
    )


def test_linear_21():
    assert d3.scale_linear().set_range([{"color": "red"}, {"color": "blue"}])(0.5), {
        "color": "rgb(128, 0, 128)"
    }
    assert d3.scale_linear().set_range([["red"], ["blue"]])(0.5) == ["rgb(128, 0, 128)"]
...
def test_linear_25():
    assert d3.scale_linear().set_range_round({1, 2}).get_range() == [1, 2]
...
def test_linear_27():
    assert d3.scale_linear().get_clamp() is False
...
def test_linear_30():
    assert d3.scale_linear().set_clamp("True").get_clamp() is True
    assert d3.scale_linear().set_clamp(1).get_clamp() is True
    assert d3.scale_linear().set_clamp("").get_clamp() is False
    assert d3.scale_linear().set_clamp(0).get_clamp() is False
...
def test_linear_32():
    assert d3.scale_linear().set_domain([0, 0.96]).nice().get_domain() == [0, 1]
    assert d3.scale_linear().set_domain([0, 96]).nice().get_domain() == [0, 100]


def test_linear_33():
    assert d3.scale_linear().set_domain([0, 0.96]).nice(10).get_domain() == [0, 1]
    assert d3.scale_linear().set_domain([0, 96]).nice(10).get_domain() == [0, 100]
    assert d3.scale_linear().set_domain([0.96, 0]).nice(10).get_domain() == [1, 0]
    assert d3.scale_linear().set_domain([96, 0]).nice(10).get_domain() == [100, 0]
    assert d3.scale_linear().set_domain([0, -0.96]).nice(10).get_domain() == [0, -1]
    assert d3.scale_linear().set_domain([0, -96]).nice(10).get_domain() == [0, -100]
    assert d3.scale_linear().set_domain([-0.96, 0]).nice(10).get_domain() == [-1, 0]
    assert d3.scale_linear().set_domain([-96, 0]).nice(10).get_domain() == [-100, 0]
    assert d3.scale_linear().set_domain([-0.1, 51.1]).nice(8).get_domain() == [-10, 60]


def test_linear_34():
    assert d3.scale_linear().set_domain([1.1, 10.9]).nice(10).get_domain() == [1, 11]
    assert d3.scale_linear().set_domain([10.9, 1.1]).nice(10).get_domain() == [11, 1]
    assert d3.scale_linear().set_domain([0.7, 11.001]).nice(10).get_domain() == [0, 12]
    assert d3.scale_linear().set_domain([123.1, 6.7]).nice(10).get_domain() == [130, 0]
    assert d3.scale_linear().set_domain([0, 0.49]).nice(10).get_domain() == [0, 0.5]
    assert d3.scale_linear().set_domain([0, 14.1]).nice(5).get_domain() == [0, 20]
    assert d3.scale_linear().set_domain([0, 15]).nice(5).get_domain() == [0, 20]
...
def test_linear_36():
    assert d3.scale_linear().set_domain([1.1, 1, 2, 3, 10.9]).nice(10).get_domain() == [
        1,
        1,
        2,
        3,
...
def test_linear_37():
    assert d3.scale_linear().set_domain([12, 87]).nice(5).get_domain() == [0, 100]
    assert d3.scale_linear().set_domain([12, 87]).nice(10).get_domain() == [10, 90]
    assert d3.scale_linear().set_domain([12, 87]).nice(100).get_domain() == [12, 87]
...
def test_linear_41():
    def check(domain, count):
        ...
        ticks = s.ticks(count)
        assert [ticks[0], ticks[-1]] == s.get_domain()
...
def test_linear_46():
    ...
    assert (
        d3.scale_linear().set_domain([0.19, 0.21]).tick_format(10, "+%")(0.2)
        == "+20.0%"
    )
...
def test_linear_47():
    ...
    assert (
        d3.scale_linear().set_domain([0, 9]).tick_format(10, "r")(2.10e6) == "2000000"
    )
    assert (
        d3.scale_linear().set_domain([0, 9]).tick_format(100, "r")(2.01e6) == "2000000"
    )
    assert (
        d3.scale_linear().set_domain([0, 9]).tick_format(100, "r")(2.11e6) == "2100000"
    )
    assert d3.scale_linear().set_domain([0, 0.9]).tick_format(10, "p")(0.210) == "20%"
    assert (
        d3.scale_linear().set_domain([0.19, 0.21]).tick_format(10, "p")(0.201)
        == "20.1%"
    )
...
def test_linear_48():
    assert (
        d3.scale_linear().set_domain([0, 1e6]).tick_format(10, "$s")(0.51e6) == "$0.5M"
    )
    assert (
        d3.scale_linear().set_domain([0, 1e6]).tick_format(100, "$s")(0.501e6)
        == "$0.50M"
    )
...
def test_linear_54():
    ...
    assert y.get_unknown() is None
    y.set_unknown(3)
    assert x(math.nan) == 2
    assert y(math.nan) == 3
    assert x.get_unknown() == 2
