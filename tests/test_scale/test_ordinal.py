...
def test_ordinal_2():
    s = d3.scale_ordinal().set_domain([0, 1]).set_range(["foo", "bar"]).set_unknown(None)
    assert s(0) == "foo"
    assert s(1) == "bar"
    s.set_range(["a", "b", "c"])
    assert s(0) == "a"
...
def test_ordinal_18():
    s = d3.scale_ordinal().set_domain(["foo", "bar"]).set_unknown("gray").set_range(["red", "blue"])
    assert s("foo") == "red"
    assert s("bar") == "blue"
    assert s("baz") == "gray"
    assert s("quux") == "gray"
...
def test_ordinal_19():
    s = d3.scale_ordinal().set_domain(["foo", "bar"]).set_unknown(None).set_range(["red", "blue"])
    assert s("baz") == "red"
    assert s.domain == ["foo", "bar", "baz"]

def test_ordinal_20():
    s1 = d3.scale_ordinal().set_domain([1, 2]).set_range(["red", "green"]).set_unknown("gray")
    s2 = s1.copy()
