...
def test_sequentialQuantile_1():
    s = d3.scale_sequential_quantile().domain([0, 1, 2, 3, 10])
    assert s(-1) == 0
    assert s(0) == 0
    assert s(1) == 0.25
    assert s(10) == 1
...
def test_sequentialQuantile_2():
    s = d3.scale_sequential_quantile().domain([0, 2, 9, 0.1, 10])
...
def test_sequentialQuantile_4():
    s = d3.scale_sequential_quantile().domain([2 * i / 1999 for i in range(2000)])
