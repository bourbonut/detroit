...
def test_diverging_4():
    ...
    assert sorted(s.get_domain()) == sorted([-1.2, 0, 2.4])


def test_diverging_5():
    s = d3.scale_diverging().set_domain([2, 2, 3])
    assert s.get_domain() == [2, 2, 3]
    assert s(-1.2) == 0.5
    assert s(0.6) == 0.5
    assert s(2.4) == 0.7
    assert s.set_domain([1, 2, 2]).get_domain() == [1, 2, 2]
    assert s(-1.0) == -1
    assert s(0.5) == -0.25
    assert s(2.4) == 0.5
    assert s.set_domain([2, 2, 2]).get_domain() == [2, 2, 2]
    assert s(-1.0) == 0.5
    assert s(0.5) == 0.5
    assert s(2.4) == 0.5
...
def test_diverging_9():
    ...
    assert s(2.4) == 0.7
    assert math.isnan(s.set_domain([1, math.nan, 2]).get_domain()[1])
    assert math.isnan(s(-1.0)) is True
    assert math.isnan(s(0.5)) is True
    assert math.isnan(s(2.4)) is True
    assert math.isnan(s.set_domain([0, 1, math.nan]).get_domain()[2])
    assert s(-1.0) == -0.5
    assert s(0.5) == 0.25
    assert math.isnan(s(2.4)) is True
...
def test_diverging_14():
    ...
    assert s.get_interpolator()(0.5) == 3
    assert s.get_range() == [1, 3, 10]
