...
def test_diverging_2():
    s = d3.scale_diverging().clamp(True)
...
def test_diverging_3():
    s = d3.scale_diverging().domain(["-1.20", " 0", "2.40"])
...
def test_diverging_4():
    s = d3.scale_diverging().domain({-1.2, 0, 2.4})
    assert s.domain() == [-1.2, 0, 2.4]

def test_diverging_5():
    s = d3.scale_diverging().domain([2, 2, 3])
    assert s.domain() == [2, 2, 3]
    assert s(-1.2) == 0.5
    assert s( 0.6) == 0.5
    assert s( 2.4) == 0.7
    assert s.domain([1, 2, 2]).domain() == [1, 2, 2]
    assert s(-1.0) == -1
    assert s( 0.5) == -0.25
    assert s( 2.4) == 0.5
    assert s.domain([2, 2, 2]).domain() == [2, 2, 2]
    assert s(-1.0) == 0.5
    assert s( 0.5) == 0.5
    assert s( 2.4) == 0.5

def test_diverging_6():
    s = d3.scale_diverging().domain([4, 2, 1])
...
def test_diverging_7():
    s = d3.scale_diverging_log().domain([3, 2, 1])
...
def test_diverging_8():
    s = d3.scale_diverging_log().domain([-1, -2, -3])
...
def test_diverging_14():
    s = d3.scale_diverging().range([1, 3, 10])
    assert s.interpolator()(0.5) == 3
    assert s.range() == [1, 3, 10]

def test_diverging_15():
    s = d3.scale_diverging().range([1, 3, 10, 20])
    assert s.interpolator()(0.5) == 3
    assert s.range() == [1, 3, 10]
