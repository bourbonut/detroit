...
def test_quantize_3():
    ...
    b = {}
    c = {}
    s = d3.scale_quantize().set_range([a, b, c])
    assert s(-0.5) == a
    assert s(+1.5) == c


def test_quantize_4():
    s = d3.scale_quantize().set_range([0, 1, 2]).set_unknown(-1)
    assert s(None) == -1
    assert s(None) == -1
    assert s(math.nan) == -1
...
def test_quantize_10():
    s = d3.scale_quantize().set_range([0, 1, 2, 3])
...
def test_quantize_12():
    s = d3.scale_quantize().set_range([0, 1, 2, 0])
    assert s.invert_extent(0) == [0.00, 0.25]
    assert s.invert_extent(1) == [0.25, 0.50]
