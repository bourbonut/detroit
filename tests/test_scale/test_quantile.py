...
def test_quantile_15():
    s = d3.scale_quantile().domain([3, 6, 7, 8, 8, 10, 13, 15, 16, 20])
    assert all(map(math.isnan, s.invertExtent(-1)))
    assert all(map(math.isnan, s.invertExtent(0.5)))
    assert all(map(math.isnan, s.invertExtent(2)))
    assert all(map(math.isnan, s.invertExtent('a')))
