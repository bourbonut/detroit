...
def test_point_3():
    ...
    b = d3.scale_band().set_domain(["foo", "bar"]).set_range([0, 960]).set_padding_inner(1)
...
def test_point_4():
    ...
    b = d3.scale_band().set_domain(["foo", "bar"]).set_range([0, 960]).set_padding_inner(0.5).set_padding_outer(0.5)
    assert list(map(p, p.domain)) == list(map(b, b.domain))
    assert p.bandwidth == b.bandwidth
    assert p.step == b.step
