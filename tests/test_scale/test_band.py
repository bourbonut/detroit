...
def test_band_5():
    s = d3.scale_band([0, 960])
    assert s.domain(["foo"]).step() == 960
    assert s.domain(["foo", "bar"]).step() == 480
    assert s.domain(["foo", "bar", "baz"]).step() == 320
    s.padding(0.5)
    assert s.domain(["foo"]).step() == 640
    assert s.domain(["foo", "bar"]).step() == 384

def test_band_6():
    s = d3.scale_band([0, 960])
    assert s.domain([]).bandwidth() == 960
    assert s.domain(["foo"]).bandwidth() == 960
    assert s.domain(["foo", "bar"]).bandwidth() == 480
    assert s.domain(["foo", "bar", "baz"]).bandwidth() == 320
    s.padding(0.5)
    assert s.domain([]).bandwidth() == 480
    assert s.domain(["foo"]).bandwidth() == 320
    assert s.domain(["foo", "bar"]).bandwidth() == 192

def test_band_7():
    s = d3.scale_band([0, 960]).domain([])
    assert s.step() == 960
...
def test_band_8():
    s = d3.scale_band([0, 960]).domain(["foo"])
    assert s("foo") == 0
    assert s.step() == 960
    assert s.bandwidth() == 960
    s.padding(0.5)
...
def test_band_11():
    domain = ["red", "green"]
    s = d3.scale_band().domain(domain)
    domain.append("blue")
    assert s.domain() == ["red", "green"]

def test_band_12():
    s = d3.scale_band().domain(["red", "green"])
    domain = s.domain()
...
def test_band_14():
    range_ = [1, 2]
    s = d3.scale_band().range(range_)
    range_.append("blue")
    assert s.range() == [1, 2]

def test_band_15():
    s = d3.scale_band().range([1, 2])
    range_ = s.range()
    assert range_ == [1, 2]
    range_.append("blue")
    assert s.range() == [1, 2]
