...
def test_band_10():
    assert sorted(d3.scale_band().set_domain({"a", "b", "c"}).domain) == sorted(["a", "b", "c"])
...
def test_band_20():
    s = d3.scale_band().set_domain(["a", "b", "c"]).set_range([120, 0]).set_padding_inner(0.1).set_round(True)
...
def test_band_22():
    s = d3.scale_band().set_domain(["a", "b", "c"]).set_range([120, 0]).set_padding_inner(0.2).set_padding_outer(0.1)
...
def test_band_26():
    s1 = d3.scale_band().set_domain(["red", "green"]).set_range([1, 2]).set_round(True).set_padding_inner(0.1).set_padding_outer(0.2)
    s2 = s1.copy()
