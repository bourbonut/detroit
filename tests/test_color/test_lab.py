...
from detroit.coloration.color import Color, RGB
...
def test_lab_2():
    assert approx_equal(d3.lab("rgba(170, 187, 204, 0.4)"), 74.96879980931759, -3.398998724348956, -10.696507207853333, 0.4)
...
def test_lab_12():
    assert approx_equal(d3.lab("#abcdef"), 80.77135418262527, -5.957098328496224, -20.785782794739237, 1)
    assert approx_equal(d3.lab("#abc"), 74.96879980931759, -3.398998724348956, -10.696507207853333, 1)
    assert approx_equal(d3.lab("rgb(12, 34, 56)"), 12.404844123471648, -2.159950219712034, -17.168132391132946, 1)
    assert approx_equal(d3.lab("rgb(12%, 34%, 56%)"), 35.48300043476593, -2.507637675606522, -36.95112983195855, 1)
    assert approx_equal(d3.lab("rgba(12%, 34%, 56%, 0.4)"), 35.48300043476593, -2.507637675606522, -36.95112983195855, 0.4)
    assert approx_equal(d3.lab("hsl(60,100%,20%)"), 41.97125732118659, -8.03835128380484, 47.65411917854332, 1)
    assert approx_equal(d3.lab("hsla(60,100%,20%,0.4)"), 41.97125732118659, -8.03835128380484, 47.65411917854332, 0.4)
    assert approx_equal(d3.lab("aliceblue"), 97.12294991108756, -1.773836604137824, -4.332680308569969, 1)
...
def test_lab_16():
    assert approx_equal(d3.lab(d3.rgb(255, 0, 0, 0.4)), 54.29173376861782, 80.8124553179771, 69.88504032350531, 0.4)

def test_lab_17():
    c = d3.lab("rgba(165, 42, 42, 0.4)")
    assert approx_equal(c.brighter(0.5), 47.149667346714935, 50.388769337115, 31.834059255569358, 0.4)
    assert approx_equal(c.brighter(1), 56.149667346714935, 50.388769337115, 31.834059255569358, 0.4)
    assert approx_equal(c.brighter(2), 74.14966734671493, 50.388769337115, 31.834059255569358, 0.4)

def test_lab_18():
    c1 = d3.lab("rgba(70, 130, 180, 0.4)")
    c2 = c1.brighter(1)
    assert approx_equal(c1, 51.98624890550498, -8.362792037014344, -32.832699449697685, 0.4)
    assert approx_equal(c2, 69.98624890550498, -8.362792037014344, -32.832699449697685, 0.4)
...
def test_lab_21():
    c = d3.lab("rgba(165, 42, 42, 0.4)")
    assert approx_equal(c.darker(0.5), 29.149667346714935, 50.388769337115, 31.834059255569358, 0.4)
    assert approx_equal(c.darker(1), 20.149667346714935, 50.388769337115, 31.834059255569358, 0.4)
    assert approx_equal(c.darker(2), 2.149667346714935, 50.388769337115, 31.834059255569358, 0.4)

def test_lab_22():
    c1 = d3.lab("rgba(70, 130, 180, 0.4)")
    c2 = c1.darker(1)
    assert approx_equal(c1, 51.98624890550498, -8.362792037014344, -32.832699449697685, 0.4)
    assert approx_equal(c2, 33.98624890550498, -8.362792037014344, -32.832699449697685, 0.4)
