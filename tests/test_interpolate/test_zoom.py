...
def test_zoom_2():
    assert math.isclose(
        d3.interpolate_zoom([0, 0, 1], [0, 0, 1.1]).duration, 67, rel_tol=1
    )
    assert math.isclose(
        d3.interpolate_zoom([0, 0, 1], [0, 0, 2]).duration, 490, rel_tol=1
    )
    assert math.isclose(
        d3.interpolate_zoom([0, 0, 1], [10, 0, 8]).duration, 2872.5, rel_tol=1
    )
