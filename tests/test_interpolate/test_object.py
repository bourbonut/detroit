...
def test_object_2():
    ...
    assert d3.interpolate_object({"stroke": "red"}, {"stroke": "green"})(0.5) == {"stroke": "rgb(128, 64, 0)"}
    assert d3.interpolate_object({"color": "red"}, {"color": "green"})(0.5) == {"color": "rgb(128, 64, 0)"}

def test_object_3():
    assert d3.interpolate_object({"foo": [2, 12]}, {"foo": [4, 24]})(0.5) == {"foo": [3, 18]}
    assert d3.interpolate_object({"foo": {"bar": [2, 12]}}, {"foo": {"bar": [4, 24]}})(0.5) == {"foo": {"bar": [3, 18]}}

def test_object_4():
    assert d3.interpolate_object({"foo": 2, "bar": 12}, {"foo": 4})(0.5) == {"foo": 3}

def test_object_5():
    assert d3.interpolate_object({"foo": 2}, {"foo": 4, "bar": 12})(0.5) == {"foo": 3, "bar": 12}
