...
def test_time_hour_floor():
  assert d3.time_hour.floor(datetime(2010, 12, 31, 23, 59)) == datetime(2010, 12, 31, 23)
...
def test_time_hour_offset():
  ...
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23, 59, 59, 999), +1) == datetime(2011,  1,  1,  0, 59, 59, 999)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23, 59, 59, 456), -2) == datetime(2010, 12, 31, 21, 59, 59, 456)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 12), -1) == datetime(2010, 12, 31, 11)
  assert d3.time_hour.offset(datetime(2011,  1,  1,  1), -2) == datetime(2010, 12, 31, 23)
  assert d3.time_hour.offset(datetime(2011,  1,  1,  0), -1) == datetime(2010, 12, 31, 23)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 11), +1) == datetime(2010, 12, 31, 12)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23), +2) == datetime(2011,  1,  1,  1)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23), +1) == datetime(2011,  1,  1,  0)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23, 59, 59, 999), 0) == datetime(2010, 12, 31, 23, 59, 59, 999)
  assert d3.time_hour.offset(datetime(2010, 12, 31, 23, 59, 58,   0), 0) == datetime(2010, 12, 31, 23, 59, 58,   0)


def test_time_hour_range():
  assert d3.time_hour.range(datetime(2010, 12, 31, 12, 30), datetime(2010, 12, 31, 15, 30)) == [
    datetime(2010, 12, 31, 13),
    datetime(2010, 12, 31, 14),
    datetime(2010, 12, 31, 15)
  ]

  assert d3.time_hour.range(datetime(2010, 12, 31, 23), datetime(2011, 1, 1, 2))[0] == datetime(2010, 12, 31, 23)
  assert d3.time_hour.range(datetime(2010, 12, 31, 23), datetime(2011, 1, 1, 2))[2] == datetime(2011, 1, 1, 1)
  ...
  assert d3.time_hour.range(datetime.fromtimestamp(1478415600), datetime.fromtimestamp(1478430000)) == [
...
def test_time_hour_every():
    ...
    assert d3.time_hour.every(4).range(datetime(2008, 12, 30, 12, 47), datetime(2008, 12, 31, 13, 57)) == expected_values
    expected_values = [datetime(2008, 12, 31, 0), datetime(2008, 12, 31, 12)]
    assert d3.time_hour.every(12).range(datetime(2008, 12, 30, 12, 47), datetime(2008, 12, 31, 13, 57)) == expected_values
