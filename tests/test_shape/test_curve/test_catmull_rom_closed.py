...
from detroit.shape.curves import curve_catmull_rom_closed

def test_catmull_rom_closed_1():
  line = d3.line().curve(curve_catmull_rom_closed)
  assert line([]) is None
...
def test_catmull_rom_closed_2():
  line = d3.line().curve(curve_catmull_rom_closed(0))
  assert line([]) is None
  assert line([[0, 1]]) == "M0,1Z"
  assert line([[0, 1], [1, 3]]) == "M1,3L0,1Z"
...
def test_catmull_rom_closed_3():
  line = d3.line().curve(curve_catmull_rom_closed(1))
  assert line([]) is None
  assert line([[0, 1]]) == "M0,1Z"
  assert line([[0, 1], [1, 3]]) == "M1,3L0,1Z"
...
def test_catmull_rom_closed_4():
  line = d3.line().curve(curve_catmull_rom_closed(0.5))
  assert d3.line().curve(curve_catmull_rom_closed)([[0, 1], [1, 3], [2, 1], [3, 3]]) == line([[0, 1], [1, 3], [2, 1], [3, 3]])

def test_catmull_rom_closed_5():
  line = d3.line().curve(curve_catmull_rom_closed(0.5))
  assert d3.line().curve(curve_catmull_rom_closed(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]) == line([[0, 1], [1, 3], [2, 1], [3, 3]])

def test_catmull_rom_closed_6():
  area = d3.area().curve(curve_catmull_rom_closed(0.5))
  assert d3.area().curve(curve_catmull_rom_closed(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]) == area([[0, 1], [1, 3], [2, 1], [3, 3]])
