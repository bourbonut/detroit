...
def test_delaunay_5():
    delaunay = d3.Delaunay.from_points([None] * 4, lambda d, i: i & 1, lambda d, i: (i >> 1) & 1)
    assert delaunay.points == [0, 0, 1, 0, 0, 1, 1, 1]
    assert delaunay.triangles == [0, 2, 1, 2, 3, 1]
    assert delaunay.halfedges == [-1, 5, -1, -1, -1, 1]
...
def test_delaunay_7():
    voronoi = d3.Delaunay.from_points([[0, 0], [1, 0], [0, 1], [1, 1]]).voronoi([-1, -1, 2, 2])
...
def test_delaunay_19():
    ...
    delaunay = d3.Delaunay.from_points([[0, 0] for _ in range(1000)] + [[10, 10], [10, -10]])
    assert delaunay.find(0, 0, 1) > -1
