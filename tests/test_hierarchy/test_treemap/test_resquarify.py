...
def test_treemap_resquarify_5():
    ...
    treemapper = d3.treemap().set_tile(d3.treemap_resquarify)
    treemapper(root.sum(lambda d: d.get("value")))
    treemapper(root.sum(lambda d: d.get("sum")))
    a = [[d.x0, d.x1, d.y0, d.y1] for d in root.leaves()]
    assert a == [[0, 1, 0, 0], [0, 1, 0, 0]]
