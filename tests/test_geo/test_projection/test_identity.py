...
def test_identity_3():
    identity = d3.geo_identity().translate([100, 10]).scale(2).set_reflect_x(False).set_reflect_y(False)
...
def test_identity_5():
    ...
    assert path({"type": "LineString", "coordinates": [[0,0], [10,10]]}) == "M5,5L10,10"
    identity.translate([30,90]).scale(2).set_reflect_y(True).set_clip_extent([[35,76], [45, 86]])
