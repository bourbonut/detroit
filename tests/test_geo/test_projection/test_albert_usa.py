...
def test_albert_usa_1():
    ...
    assert projection_equal(
        albers_usa, [-122.4194, 37.7749], [107.4, 214.1], 0.1
    )  # San Francisco, CA
    assert projection_equal(
        albers_usa, [-74.0059, 40.7128], [794.6, 176.5], 0.1
    )  # New York, NY
    assert projection_equal(
        albers_usa, [-95.9928, 36.1540], [488.8, 298.0], 0.1
    )  # Tulsa, OK
    assert projection_equal(
        albers_usa, [-149.9003, 61.2181], [171.2, 446.9], 0.1
    )  # Anchorage, AK
    assert projection_equal(
        albers_usa, [-157.8583, 21.3069], [298.5, 451.0], 0.1
    )  # Honolulu, HI
