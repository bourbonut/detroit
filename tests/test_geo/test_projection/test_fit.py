...
def test_fit_5(world):
    projection = (
        d3.geo_conic_conformal().set_clip_angle(30).parallels([30, 60]).rotate([0, -45])
    )
    projection.fit_extent([[50, 50], [950, 950]], world)
...
def test_fit_17():
    ...
    projection.fit_extent(
        [[50, 50], [950, 950]], {"type": "MultiPoint", "coordinates": []}
    )
    s = projection.get_scale()
    t = projection.get_translation()
    assert not s
...
def test_fit_18():
    projection = d3.geo_equirectangular()
    projection.fit_extent(
        [[50, 50], [950, 950]], {"type": "MultiLineString", "coordinates": []}
    )
    s = projection.get_scale()
    t = projection.get_translation()
    assert not s
...
def test_fit_19():
    projection = d3.geo_equirectangular()
    projection.fit_extent(
        [[50, 50], [950, 950]], {"type": "MultiPolygon", "coordinates": []}
    )
    s = projection.get_scale()
    t = projection.get_translation()
    assert not s
...
def test_fit_21(world):
    ...
    p2 = (
        ...
        .set_clip_extent([[100, 200], [700, 600]])
        .fit_size([1000, 1000], world)
    )
    s2 = p2.get_scale()
...
def test_fit_22(world):
    projection = (
        ...
        .fit_extent([[50, 50], [950, 950]], world)
        .scale(500)
    )
...
def test_fit_23(world):
    box = {
        "type": "Polygon",
        "coordinates": [[[-135, 45], [-45, 45], [-45, -45], [-135, -45], [-135, 45]]],
    }
