...
def test_contains_3():
    assert (
        d3.geo_contains({"type": "MultiPoint", "coordinates": [[0, 0], [1, 2]]}, [0, 0])
        is True
    )
    assert (
        d3.geo_contains({"type": "MultiPoint", "coordinates": [[0, 0], [1, 2]]}, [1, 2])
        is True
    )
    assert (
        d3.geo_contains({"type": "MultiPoint", "coordinates": [[0, 0], [1, 2]]}, [1, 3])
        is False
    )
...
def test_contains_4():
    assert (
        d3.geo_contains({"type": "LineString", "coordinates": [[0, 0], [1, 2]]}, [0, 0])
        is True
    )
    assert (
        d3.geo_contains({"type": "LineString", "coordinates": [[0, 0], [1, 2]]}, [1, 2])
        is True
    )
    assert (
        d3.geo_contains(
            {"type": "LineString", "coordinates": [[0, 0], [1, 2]]},
            d3.geo_interpolate([0, 0], [1, 2])(0.3),
        )
        is True
    )
    assert (
        d3.geo_contains(
            {"type": "LineString", "coordinates": [[0, 0], [1, 2]]},
            d3.geo_interpolate([0, 0], [1, 2])(1.3),
        )
        is False
    )
    assert (
...
def test_contains_14():
    ...
    feature_collection = {"type": "FeatureCollection", "features": [feature1, feature2]}
