...
def centroid(projection, obj):
    return d3.geo_path().set_projection(projection).centroid(obj)


def test_centroid_1(equirectangular):
    assert centroid(equirectangular, {"type": "Point", "coordinates": [0, 0]}) == [
        480,
        250,
    ]
...
def test_centroid_2(equirectangular):
    assert all(
        map(isnan, centroid(equirectangular, {"type": "MultiPoint", "coordinates": []}))
    )


def test_centroid_3(equirectangular):
    assert centroid(
        equirectangular, {"type": "MultiPoint", "coordinates": [[0, 0]]}
    ) == [480, 250]


def test_centroid_4(equirectangular):
    assert centroid(
        equirectangular, {"type": "MultiPoint", "coordinates": [[-122, 37], [-74, 40]]}
    ) == [-10, 57.5]


def test_centroid_5(equirectangular):
    assert all(
        map(isnan, centroid(equirectangular, {"type": "LineString", "coordinates": []}))
    )


def test_centroid_6(equirectangular):
    assert centroid(
        equirectangular, {"type": "LineString", "coordinates": [[100, 0], [0, 0]]}
    ) == [730, 250]
    assert centroid(
        equirectangular,
        {"type": "LineString", "coordinates": [[0, 0], [100, 0], [101, 0]]},
    ) == [732.5, 250]


def test_centroid_7(equirectangular):
    assert centroid(
        equirectangular, {"type": "LineString", "coordinates": [[-122, 37], [-122, 37]]}
    ) == [-130, 65]
    assert centroid(
        equirectangular, {"type": "LineString", "coordinates": [[-74, 40], [-74, 40]]}
    ) == [110, 50]
...
def test_centroid_8(equirectangular):
    assert centroid(
        equirectangular,
        {"type": "LineString", "coordinates": [[-122, 37], [-74, 40], [-74, 40]]},
    ) == [-10, 57.5]
...
def test_centroid_10(equirectangular):
    assert centroid(
        equirectangular,
        {
            ...
            "coordinates": [[[100, 0], [0, 0]], [[-10, 0], [0, 0]]],
        },
    ) == [705, 250]
...
def test_centroid_12(equirectangular):
    assert centroid(
        equirectangular,
        {"type": "Polygon", "coordinates": [[[1, 0], [2, 0], [3, 0], [1, 0]]]},
    ) == [490, 250]
...
def test_centroid_15(equirectangular):
    assert all(
        map(
            isnan,
            centroid(equirectangular, {"type": "MultiPolygon", "coordinates": []}),
        )
    )
