...
def test_graticule_18():
    ...
    for line in lines:
        assert d3.extent(line["coordinates"], lambda p: p[1]) == [-80 - 1e-6, +80 + 1e-6]
...
def test_graticule_19():
    ...
    for line in lines:
        assert d3.extent(line["coordinates"], lambda p: p[1]) == [-90 + 1e-6, +90 - 1e-6]
...
def test_graticule_22():
    g = d3.geo_graticule().set_extent([[-90, -45], [90, 45]]).set_step([45, 45]).set_precision(3)
    assert g() == {"type": "MultiLineString", "coordinates": list(map(lambda line: line["coordinates"], g.lines()))}
